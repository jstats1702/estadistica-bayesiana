...
&= \int_0^1 \theta^1 (1-\theta)^{1 - 1}\,  \theta^{a + y - 1} (1 - \theta)^{b + n - y}\, \text{d}\theta \\
...
Raiffa y Schlaifer (1961) introdujeron la conjugación para distribuciones previas, con aportes clave de Diaconis y Ylvisaker (1979, 1985). Los intervalos de credibilidad Bayesianos también exhiben propiedades de cobertura frecuentista (Welch & Peers, 1963), lo que algunos proponen como criterio para la selección de distribuciones previas (Tibshirani, 1989, Sweeting, 1999). Kass y Wasserman (1996) presentan una revisión de métodos formales para esta selección. La familia exponencial es clave porque permite distribuciones a priori conjugadas, simplificando la actualización de la posterior y facilitando cálculos analíticos. Esto optimiza la inferencia probabilística y la implementación de métodos computacionales eficientes.
...
- Se desea estimar la probabilidad \( \theta \) de reincidencia en adolescentes con base en un estudio en el que se observaron \( n = 43 \) individuos liberados de reclusión, de los cuales \( y = 15 \) reincidieron en un período de 36 meses.  
     ...
     6. Demuestre que si \( y \mid \theta \) sigue una distribución exponencial con parámetro \( \theta \), entonces la distribución \( \textsf{Gamma} \) es una distribución previa conjugada para la inferencia sobre \( \theta \), dada una muestra aleatoria de valores de \( y \).
