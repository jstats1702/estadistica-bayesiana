...
- Aprox. el 99\% de los puntajes promedio de los departamentos están distanciados a 3 × 8 = 24 puntos entre sí.
...
```{r, echo = F, fig.height=3.5, fig.width=4.5, fig.align='center'}
...
```{r, echo = F, fig.width=7, fig.height=10, fig.align='center'}
# cv Bayesiano
ids2 <- estadisticos$nombre
...
cv   <- 100*shat/that
# gráfico
plot(1:m, cv, xlab = "", ylab = "CV(%)", main = "CV Frecuentista", pch = 19, type = "b", xlim = c(1,m), ylim = c(0,20), cex.axis = 0.75, xaxt = "n")
axis(side = 1, at = 1:m, labels = F)
text(x = (1:m) + 0.3, y = par("usr")[3] - 1.5, labels = ids2, srt = 70, pos = 2, xpd = T, cex = 0.75)
abline(v = 1:m, col = "gray95", lwd = 1, lty = 3)
abline(h = 5,  lty = 2, col = 3)
