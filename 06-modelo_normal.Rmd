...
```{r,echo=FALSE}
nombres <- as.character(c(20151,20152,
                          20161,20162,
                          20171,20172,
                          20181,20182,
                          20191,20192,
...
for (i in 1:length(nombres)) {
     # datos
     dat <- read.csv(paste0("C:/Users/User/Dropbox/UN/estadistica_bayesiana/SB11_",nombres[i],"_muestra.txt"), sep=";")
     ...
     est_B  <- mean(theta)
     cv_B   <- sd(theta)/mean(theta)
     ic_B_1 <- quantile(x = theta, probs = c(.025,.975))
     ic_B_2 <- quantile(x = theta, probs = c(.005,.995))
     # resultados
     out <- rbind(out, c(n, est_B, cv_B, ic_B_1, ic_B_2))
}
colnames(out) <- c("n", "Estimación", "CV", "L. Inf. 95%", "L. Sup. 95%", "L. Inf. 99%", "L. Sup. 99%")
rownames(out) <- c("2015-1","2015-2",
                   "2016-1","2016-2",
                   "2017-1","2017-2",
                   "2018-1","2018-2",
                   "2019-1","2019-2",
...
plot(x = 1:nrow(out), y = out[,2], ylim = c(50,85), pch = 16, cex = 1, xaxt = "n", xlab = "Semestre", ylab = expression(theta))
lines(x = 1:nrow(out), y = out[,2], type = "l", col = 4)
abline(h = 50, col = 1, lty = 2)
abline(v = 1:nrow(out), col = "gray95")
segments(x0 = 1:nrow(out), y0 = out[,4], x1 = 1:nrow(out), y1 = out[,5], lwd = 3)
segments(x0 = 1:nrow(out), y0 = out[,6], x1 = 1:nrow(out), y1 = out[,7], lwd = 1)
axis(side = 1, at = 1:nrow(out), labels = FALSE)
text(x = 1:nrow(out),
     y = par("usr")[3] - 2.5,
     labels = rownames(out),
     xpd = NA,
...
```{r, echo=F, fig.align='center'}
plot(x = 1:nrow(out), y = out[,3], ylim = c(0,0.25), pch = 20, cex = 1, xaxt = "n", xlab = "Semestre", ylab = "Coef. Variación")
abline(v = 1:nrow(out), col = "gray95")
abline(h = 0.05, lty = 2, col = 3)
abline(h = 0.10, lty = 2, col = "#FFA500")
abline(h = 0.15, lty = 2, col = 2)
lines(x = 1:nrow(out), y = out[,3], type = "l", col = 4)
lines(x = 1:nrow(out), y = out[,3], type = "p", pch = 20, cex = 1.1)
axis(side = 1, at = 1:nrow(out), labels = FALSE)
text(x = 1:nrow(out),
     y = par("usr")[3] - 0.02,
     labels = rownames(out),
