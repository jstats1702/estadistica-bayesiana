...
Considere especificar el estado de información previo sobre $\theta$ de manera **independiente** de $\sigma^2$, de modo que $p(\theta,\sigma^2)=p(\theta)\,p(\sigma^2)$,  
donde 
$$
...
En el caso de una prior conjugada, donde $\tau_0^2$ es proporcional a $\sigma^2$, la distribución posterior de $\sigma^2$ sigue una Gamma Inversa. Sin embargo, en este caso, **la distribución posterior de $\sigma^2$ no corresponde a una distribución estándar conocida**.
...
Como punto de partida, basta con asignar un **valor inicial** a $\sigma^2$. Usualmente, este valor se genera a partir de la distribución previa, es decir, $\sigma^{2\,(0)}\sim\textsf{IG}\left(\tfrac{\nu_0}{2},\tfrac{\nu_0\sigma^2_0}{2}\right)$.
...
- **Irreducibilidad:** Cualquier estado $i$ puede alcanzarse desde cualquier otro estado $j$ en un número finito de pasos con probabilidad positiva. Formalmente, existe algún $n$ tal que $\text p^n(i, j) > 0$, donde $\text p^n(i, j)$ es la probabilidad de transición de $i$ a $j$ en $n$ pasos.  

- **Aperiodicidad:** No existe un período fijo $d > 1$ que restrinja las visitas a cada estado a múltiplos de $d$. En términos formales, para todo estado $i$, el máximo común divisor de los tiempos de retorno a $i$ es 1.  

- **Recurrencia positiva:** Todo estado $i$ tiene un tiempo de retorno esperado finito, es decir, $\textsf{E}[\tau_i] < \infty$, donde $\tau_i$ es el tiempo que transcurre hasta que la cadena regresa a $i$.  
...
$$
W = \frac{1}{M} \sum_{m=1}^{M} s_m^2, \quad \text{donde} \quad s_m^2 = \frac{1}{B-1} \sum_{b=1}^{B} (\theta_m^{(b)} - \bar{\theta}_m)^2.
$$  
y la **varianza entre las cadenas** como  
$$
B = \frac{B}{M-1} \sum_{m=1}^{M} (\bar{\theta}_m - \bar{\theta})^2, \quad \text{donde} \quad \bar{\theta} = \frac{1}{M} \sum_{m=1}^{M} \bar{\theta}_m.
$$  

A partir de esto, el **estimador de la varianza total** es  
$$
\hat{\sigma}^2 = \frac{B}{B-1} W + \frac{1}{B} B,
$$  
...
El término *muestreador de Gibbs* fue introducido por Geman y Geman (1984) en análisis de imágenes, aunque el algoritmo ya aparecía en estadística espacial, como en Besag (1974) y Ripley (1979). Su utilidad para inferencia Bayesiana se destacó a finales de los 80 con Gelfand y Smith (1990). Para una revisión histórica, ver Robert y Casella (2008). Evaluar la convergencia del muestreo de Gibbs y la precisión de MCMC es difícil. Se han propuesto diagnósticos (Gelman y Rubin, 1992; Geweke, 1992; Raftery y Lewis, 1992), aunque solo detectan problemas sin garantizar una buena aproximación (Geyer, 1992).

# Ejercicios

## Ejercicios conceptuales

- Considere el modelo Normal  
$$  
y_i \mid \theta, \sigma^2 \overset{\text{iid}}{\sim} \textsf{N}(\theta, \sigma^2), \quad i = 1, \dots, n,  
$$  
con distribución previa independiente \( p(\theta,\sigma^2) = p(\theta) p(\sigma^2) \), donde  
$$  
\theta \sim \textsf{N}(\mu_0, \tau^2_0), \qquad \sigma^2 \sim \textsf{GI}\left(\frac{\nu_0}{2}, \frac{\nu_0\sigma^2_0}{2} \right),  
$$  
siendo \( \mu_0 \), \( \tau^2_0 \), \( \nu_0 \) y \( \sigma^2_0 \) los hiperparámetros del modelo.

a. Determine la distribución condicional completa de $\theta$.
b. Determine la distribución condicional completa de $\sigma^2$.
c. Demuestre que 
$$
\sum_{i=1}^n (y_i - \theta)^2 = (n-1)s^2 + n(\bar{y} - \theta)^2,
$$
donde $\bar{y}$ es la media muestral y $s^2 = \frac{1}{n-1}\sum_{i=1}^n (y_i - \bar{y})^2$ es la varianza muestral.
...
- Considere los datos sobre el número de hijos de hombres en sus 30s, con y sin título universitario, reportados en `menchild30bach.dat` y `menchild30nobach.dat`. Asuma modelos Poisson para ambos grupos, pero ahora parametrizamos \( \theta_A \) y \( \theta_B \) como \( \theta_A = \theta \) y \( \theta_B = \theta \times \phi \), donde \( \phi \) representa la razón relativa \( \theta_B / \theta_A \). Suponemos distribuciones previas \( \theta \sim \textsf{Gamma}(a_\theta, b_\theta) \) y \( \phi \sim \textsf{Gamma}(a_\phi, b_\phi) \).
     ...
     e. Estime \( \textsf{E}(\theta_B - \theta_A \mid \boldsymbol{y}_A, \boldsymbol{y}_B) \) en cada coso. Describa los efectos de la distribución previa de \( \phi \) sobre los resultados
...
- El archivo `glucose.dat` contiene las concentraciones de glucosa en plasma de 532 mujeres en un estudio sobre diabetes.
     ...
     c. Fijando \( \alpha_0 = \beta_0 = 1 \), \( \mu_0 = 120 \), \( \tau_0^2 = 200 \), \( \sigma_0^2 = 1000 \) y \( \nu_0 = 10 \), implemente un muestreador de Gibbs durante al menos 10,000 iteraciones. Realice un análisis exhaustivo de convergencia.
