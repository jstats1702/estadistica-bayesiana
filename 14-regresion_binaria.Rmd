...
*Co-clustering probabilities* estimadas a partir del **agrupamiento de los parámetros de sociabilidad** \(\delta_i\) en cada iteración del muestreador de Gibbs. El procedimiento utiliza el algoritmo **k-means** para asignar grupos en cada iteración y calcula las probabilidades de pertenencia conjunta al mismo grupo.
...
```{r}
# Calcular co-clustering probabilities
coclustering_probs <- compute_coclustering(samples)
```


```{r, echo=FALSE, fig.height = 5, fig.width = 15, fig.align='center'}
# Librerías necesarias
suppressMessages(suppressWarnings(library(ggplot2)))
suppressMessages(suppressWarnings(library(reshape2)))
suppressMessages(suppressWarnings(library(gridExtra)))
...
La **validación cruzada** implementada divide los elementos de la matriz de adyacencia \(Y\) (triangular superior) en dos subconjuntos: **entrenamiento (70%)** y **validación (30%)**. Los valores faltantes en el conjunto de entrenamiento se tratan como **datos latentes** y se actualizan iterativamente mediante un **muestreador de Gibbs**, donde en cada iteración se muestrean según la distribución muestral para mantener la coherencia del modelo. 
...
```{r}
# Dividir la matriz de adyacencia en entrenamiento y validación
set.seed(123)  # Para reproducibilidad
n <- nrow(Y)   # Número de nodos
indices <- which(upper.tri(Y, diag = FALSE), arr.ind = TRUE) # Triangular superior
n_edges <- nrow(indices)  # Número de aristas
# Selección aleatoria de entrenamiento (70%) y validación (30%)
train_indices <- sample(1:n_edges, size = round(0.7 * n_edges))
valid_indices <- setdiff(1:n_edges, train_indices)
# Crear matrices de entrenamiento y validación
Y_train <- Y
Y_valid <- Y
# Marcar valores faltantes (0.5 = valores no observados)
for (idx in valid_indices) {
  i <- indices[idx, 1]
  j <- indices[idx, 2]
  Y_train[i, j] <- NA
  Y_train[j, i] <- NA # Simetría
}
```


```{r}
# Muestreador de Gibbs con almacenamiento solo de Y_missing
gibbs_sampler_missing <- function(Y, n_iter, n_burn, n_thin) {
  # Inicialización
  mu <- 0
  delta <- rnorm(n, 0, 1)
  sigma2 <- 1
  tau2 <- 1
  z <- matrix(0, n, n)  # Variables auxiliares
  # Almacenamiento de Y_missing
  n_samples <- (n_iter - n_burn) / n_thin
  Y_missing_samples <- matrix(0, nrow = n_samples, ncol = length(valid_indices))
  # Muestreo Gibbs
  for (t in 1:n_iter) {
    # 1. Muestrear datos faltantes (Y_train == NA)
    for (idx in valid_indices) {
      i <- indices[idx, 1]
      j <- indices[idx, 2]
      eta_ij <- mu + delta[i] + delta[j]
      prob_ij <- pnorm(eta_ij)  # Probit link
      Y_train[i, j] <- rbinom(1, 1, prob_ij)
      Y_train[j, i] <- Y_train[i, j] # Simetría
    }
    # 2. Actualizar parámetros del modelo
    z <- sample_z(Y_train, mu, delta, z)
    mu <- sample_mu(z, delta, sigma2)
    delta <- sample_delta(z, mu, tau2, delta)
    sigma2 <- sample_sigma2(mu)
    tau2 <- sample_tau2(delta)
    # Almacenar muestras después de burn-in y thinning
    if (t > n_burn && (t - n_burn) %% n_thin == 0) {
      idx_sample <- (t - n_burn) / n_thin
      # Guardar valores muestreados para los datos faltantes
...
```{r}
# Configuración del muestreador de Gibbs
n_iter <- 51000
n_burn <- 1000
n_thin <- 5
Y_missing_samples <- gibbs_sampler_missing(Y, n_iter, n_burn, n_thin)
save(Y_missing_samples, file = "samples_regresion_binaria_lazega_vc.RData")
```


```{r, echo=F, eval=T}
load("C:/Users/User/Dropbox/UN/estadistica_bayesiana/samples_regresion_binaria_lazega_vc.RData")
```
...
# Evaluar modelo: Curva ROC y AUC
Y_valid_values <- Y[indices[valid_indices, ]]  # Valores verdaderos
roc_curve <- roc(Y_valid_values, prob_missing)  # Curva ROC
auc_value <- auc(roc_curve)  # Área bajo la curva (AUC)
# Mostrar resultados
cat("Área bajo la curva (AUC):", auc_value, "\n")
# Extraer coordenadas TPR y FPR
roc_data <- data.frame(
  FPR = 1 - roc_curve$specificities,  # 1 - Especificidad
  TPR = roc_curve$sensitivities       # Sensibilidad
)
# Graficar la curva ROC
library(ggplot2)
ggplot(roc_data, aes(x = FPR, y = TPR)) +
  geom_line(color = "blue", size = 1) +                           # Línea de la curva ROC
  geom_abline(slope = 1, intercept = 0, linetype = "dashed",      # Línea aleatoria (diagonal)
              color = "red") +
  labs(
    title = "Curva ROC",
