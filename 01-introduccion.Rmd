...
A mayor prevalencia, se recomienda implementar más medidas de salud pública.
...
Los modelos Bayesianos son naturalmente **jerárquicos**.
...
La distribución posterior $p(\theta \mid y)$ proporciona un marco para inferir y actualizar el conocimiento sobre $\theta$ a nivel de toda la ciudad.
...
- Tendencia central:
$$
\textsf{E}(\theta\mid\ y) = \int_\Theta y\,p(\theta\mid y)\,\text{d}y = \frac{a_n}{a_n+b_n} = 0.0909.
$$
- Variabilidad:
$$
\textsf{Var}(\theta\mid\ y) = \int_\Theta (\theta - \textsf{E}(\theta\mid\ y))^2\,p(\theta\mid y)\,\text{d}y = \frac{a_n\,b_n}{(a_n+b_n)^2(a_n+b_n+1)} = 0.0036.
$$
...
El concepto de probabilidad como una medida de incertidumbre para cantidades desconocidas pero determinísticas se remonta a obras históricas como *An Essay towards Solving a Problem in the Doctrine of Chances* de Bayes (1763) y *A Philosophical Essay on Probabilities* de Laplace (1814). A lo largo del siglo XX, hubo debates sobre el papel de la opinión previa en la inferencia estadística. Algunas discusiones más equilibradas incluyen Savage (1962), que presenta un diálogo entre estadísticos como Bartlett, Barnard, Cox, Pearson y Smith. Little (2006) examina las fortalezas y debilidades de los criterios estadísticos Bayesianos y frecuentistas, mientras que Efron (2005) revisa la evolución histórica de las filosofías estadísticas y especula sobre la futura interacción entre los métodos Bayesianos y no Bayesianos.
