...
for (i in 1:ng) {
  for (j in 1:ng) {
    lon <- gx[i]
    lat <- gy[j]
    type_dummy <- rbind(rep(0,3),diag(3)) # Matriz de variables dummys
    x <- cbind(1,type_dummy,lat,lat*type_dummy,lon,lon*type_dummy) # Matriz de diseño donde cada fila corresponde a un grupo
    for (b in 1:niter) {
      bet  <- muestras$BET[b,]
      sig2 <- muestras$SIG2[b,]
      z1[i,j,b] <- rnorm(n = 1, mean = sum(x[1,]*bet), sd = sqrt(sig2[j1]/nj[j1]))
      z2[i,j,b] <- rnorm(n = 1, mean = sum(x[2,]*bet), sd = sqrt(sig2[j2]/nj[j2]))
      z3[i,j,b] <- rnorm(n = 1, mean = sum(x[3,]*bet), sd = sqrt(sig2[j3]/nj[j3]))
      z4[i,j,b] <- rnorm(n = 1, mean = sum(x[4,]*bet), sd = sqrt(sig2[j4]/nj[j4]))
    }
  }
}
```
...
filled.contour(x = gx, y = gy, z = apply(X = z1, MARGIN = c(1,2), FUN = mean), xlab = 'Latitud', ylab = 'Longitud', main = 'bucket', las=0)
```
...
fields::image.plot(surf, col = col.br(100), xlim = range(coords[,1]), ylim = range(coords[,2]), xaxs = 'r', yaxs='r', xlab = 'Latitud', ylab = 'Longitud', main = 'bucket')
